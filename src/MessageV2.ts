...
export class Message {
  ...
  private translate_text?: string;
  private footer_hashtag_line?: string;
  ...
  private async sendToTelegram(recipient: ITelegramRecipient, fetch_blob = false): Promise<void> {
    ...
    let res;
    try {
      res = await this.sendMediaGroup({ recipient, outbound_text: outbound_text[0]!, media: this.media! });
  ...
  private getFooterHashtags() {
    return [this.app.app_name];
  }

  private getOutboundText(translate: boolean, max_len: number) {
